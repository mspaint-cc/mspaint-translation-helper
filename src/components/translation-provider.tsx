...
const baseURL = "https://raw.githubusercontent.com/mspaint-cc/translations/refs/heads/main/";

export function TranslationProvider({ children }: {children: React.ReactNode}) {
    const [normalizedTranslations, setNormalizedTranslations] = React.useState<NormalizedTranslation>({
        "en": {
            "NativeName": "English",
            "EnglishName": "English"
        }
    ...
    const [latestTranslation, setLatestTranslation] = React.useState<Record<string, string>>({
        "No message today... 😢": ""
    });
    const [currentLanguageData, setCurrentLanguageData] = React.useState<Record<string, string> | undefined>(undefined);
    const [loading, setLoading] = React.useState<boolean>(true);
    const [currentLanguageDataLoading, setCurrentLanguageDataLoading] = React.useState<boolean>(true);
    const [selectedLanguage, setSelectedLanguage] = React.useState<string>("en");
    const [showNewTranslationModal, setShowNewTranslationModal] = React.useState<boolean>(false);
    const [refreshKey, setRefreshKey] = React.useState<number>(0);

    const [internalStartupRefreshKey, setInternalStartupRefreshKey] = React.useState<number>(0);
    ...
    React.useEffect(() => {
        ...
        fetch(`${baseURL}/translations/${selectedLanguage.replaceAll("-", "/")}.json`, {
            next: {
    ...
    React.useEffect(() => {
        const fetchData = async () => {
            ...
            for (const languageTag in languagesData) {
                ...
                if (language.Default) {
                    for (const localeTag in language) {
                        if (localeTag == "Default")
                            continue;

                        normalizedTranslations[languageTag + "-" + localeTag] = language[localeTag];
                    }
            ...
            setNormalizedTranslations(normalizedTranslations);
            ...
            const currentLanguageRequest = await fetch(`${baseURL}/translations/${selectedLanguage.replaceAll("-", "/")}.json`, {
                next: {
                    revalidate: 60 // revalidate every 60 seconds
                }
            });
